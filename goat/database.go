...
func (db *dbw) GetFileRecordPeerList(infohash, exclude string, limit int) ([]byte, error) {
	...
	for rows.Next() {
		...
		ip := [4]byte{}
		binary.BigEndian.PutUint32(ip[:], binary.BigEndian.Uint32(net.ParseIP(result.IP).To4()))

		// Parse port into byte buffer
		port := [2]byte{}
		binary.BigEndian.PutUint16(port[:], result.Port)
...
func (db *dbw) GetInactiveUserInfo(fid int, _interval time.Duration) (users []userinfo, err error) {
	query := `SELECT user_id, ip FROM files_users
		WHERE time < (UNIX_TIMESTAMP() - ?)
		AND active = 1
		AND file_id = ?;`
	result := userinfo{}
	interval := int(_interval / time.Second)
	var rows *sqlx.Rows
